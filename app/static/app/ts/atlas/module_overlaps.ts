    ...
    if (type === "display") {
        ...
        if (dataset) {
            ...
            const d1Link = linkElement(
                d1Label,
                getDataPortalUrl("atlas_gene", { dataset: dataset1, gene }),
            );
            const d2Link = linkElement(
                d2Label,
                getDataPortalUrl("atlas_gene", { dataset: dataset2, gene }),
            );
...
export function loadModuleGeneTable(
    ...
    const url = getRestUrl("rest:genemodulesimilaritygenes-list", {
        dataset: dataset1,
        limit: 0,
        list_genes: 1,
        module: data.module,
