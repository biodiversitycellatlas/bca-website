    ...
    $("#bca-search").selectize({
        ...
        load: function(query, callback) {
            ...
            Promise.all([
                ...
                const options = dataset_data.results.map(
                    item => ({ ...item, group: 'dataset', name: item.species })
                );
                this.clearOptions();
                this.optgroups = {
                    dataset: {
        ...
        onChange: function(value) {
            ...
            if (item.group === 'gene') {
                ...
                window.location.href = urls.atlasView.replace('dataset_placeholder', dataset);
            }
        },
        optgroupField: 'group'
    });
