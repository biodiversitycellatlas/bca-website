...
function plotGeneExpressionComparison (id, species, gene, gene2, url, stats) {
    ...
    fetch(apiURL)
        ...
        .then(data => {
            createExpressionComparisonPlot(`#${id}-plot`, gene, gene2, data, stats);
        })
        .catch(error => {
...
function createExpressionComparisonPlot(id, gene, gene2, data, stats) {
    var chart = {
  	    ...
  	    "encoding": {
  	        "x": {
  	            "field": gene, "type": "quantitative",
  	            "title": gene + " fold-change",
  	            "scale": {"domain": {"expr": "[data('data_0')[0]['min'], data('data_0')[0]['max']]"}}
            },
  	        "y": {
  	            "field": gene2, "type": "quantitative",
  	    ...
  	    "layer": [{
  	        // Add regression line
            "mark": { "type": "line", "color": "gray", "strokeWidth": 1.5, "clip": true },
