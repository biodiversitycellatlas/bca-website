...
function setSavedFormat(id) {
    // Get valid formats
    let valid = $(`input[name="data_format_${id}"]`).map((_, el) => el.value).get();
...
export function appendDataMenu(id, urls, labels) {
    ...
    let lastIndex = $(`ul[id="data_dropdown_${id}"] > li[name="data_download_option"]`).last().data('index');
...
export function updateDataMenu(id, url, label) {
    let li = $(`ul[id="data_dropdown_${id}"] > li[name="data_download_option"] > label:contains("${label}")`).last().parent()[0];
    if (li === undefined) {
