...
from app.models import Dataset, Gene, MetacellTypeSimilarity
...
pair_columns = [
    ...
    if not c.endswith("_pval1") and not c.endswith("_pval2")
]
...
for column in pair_columns:
    ...
    print(
        f"Updated {source.name} -> {target.name}: "
        f"{len(gene_pairs)} gene pairs"
    )
